...
el-admin-layout的scss变量中使用了不少element-ui的值，大部分以`'$--'`开头的都是element-ui原有的
...
::: tip 注意
如果修改了`$max-mobile-width`，请同时修改[内部常量](./内部常量.md)中的`maxMobileWidth`

如果修改了`$menu-padding`，请同时修改[asideStore](./数据控制.md#asidestore)中的`inlineIndent`
:::
